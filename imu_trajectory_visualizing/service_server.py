...
from imu_trajectory_visualizing import ImuProcessor
...
class ServiceServer:
    ...
    def start_uvicorn_server(self):
        LOGGER.info(f'start uvicorn server on {9003} port')

        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)

        # Configure and run the server
        uvicorn_config = uvicorn.Config(app=self.app, host="0.0.0.0", port=9003, log_level="debug")
