...
if __name__ == '__main__':
    ...
    for batch in batch_select:
        ...
        print('############################################')
        print(f'testing for time consuming of batch size {batch}..')
        for i in range(repeat_count):
            for video in os.listdir(video_dir):
                video_path = os.path.join(video_dir, video)
                video_cap = cv2.VideoCapture(video_path)
                frame_number = video_cap.get(cv2.CAP_PROP_FRAME_COUNT)
                read_cnt = int(frame_number // batch_select[-1] * batch_select[-1])
                # print(read_cnt)
                frames_buffer = []
                for _ in range(read_cnt):
                    ret, frame = video_cap.read()
                    assert ret, 'invalid frame number computation'
                    frames_buffer.append(frame)
                    if len(frames_buffer) == batch:
                        start_time = time.time()
                        estimator(frames_buffer)
                        end_time = time.time()
                        process_time = (end_time - start_time) * 1000
                        time_buffer.append(process_time)
                        print(
                            f'batch size: {batch}    batch time: {process_time}ms    average frame time: {process_time / batch}ms'
                        )
                        frames_buffer = []
    ...
    print('******************************************')
    print('*************final result*****************')
    for batch, frame_time in zip(batch_select, avg_time):
        print(f'batch:{batch}  time:{frame_time}ms')
    print('******************************************')
    print('******************************************')
